...
namespace CDMD_Project.ViewModel
{
    ...
    public class PatientViewModel : ViewModelBase
    {
        ...
        public string OperationText { get { return CrudOperation == 1 ? "Register" : "Update"; } }
            ...
            try
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    //_repo.Create(newPatient);
                     List<WearableDevice> devices = new List<WearableDevice>();
                    //var patient_id = newPatient.PatientID;
                    if (Device1)
                    {
                    ...
                    if (Device4)
                    {
                        ...
                        devices.Add(wDevice4);
                    }
                    _repo.Create(newPatient, devices);
        ...
        private void _Update()
        {
            ...
            if (existingPatient != null)
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    foreach (var device in existingPatient.WearableDevices)
                    {
                        var foundDevice = devices.Find(d => d.DeviceType == device.DeviceType);
                        foundDevice.DeviceID = device.DeviceID;
                    }

                    _repo.Update(existingPatient, devices);
